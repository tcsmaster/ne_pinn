...
for el in itertools.product(gamma_2_list, gamma_3_list):
    test_data = torch.stack(torch.meshgrid(torch.tensor([x],
                                                        dtype=torch.float32
                                                       )
                                                       .squeeze(),
                                           torch.tensor([t],
    ...
    for gamma_1 in gamma_1_list:
        ...
        rmse_error[gamma_2_list.index(el[0]),
                   gamma_3_list.index(el[1]),
                   gamma_1_list.index(gamma_1)
                  ] = rmse_vec_error(pred, true_sol)
        rel_l2_error[gamma_2_list.index(el[0]),
                     gamma_3_list.index(el[1]),
                     gamma_1_list.index(gamma_1)
                    ] = l2_relative_loss(pred, true_sol)
...
pd.DataFrame(out_rmse,
             index = [f"gamma_1 = {gamma_1}" for gamma_1 in gamma_1_list],
             columns =pd.MultiIndex.from_product(
                      ([f"gamma_2 = {gamma_2}" for gamma_2 in gamma_2_list],
                       [f"gamma_3 = {gamma_3}" for gamma_3 in gamma_3_list]
                      )
                      )
            )
            .to_csv(err_dir + f"{pde}_3layer_rmse_table.csv")
pd.DataFrame(out_rel_l2,
             index = [f"gamma_1 = {gamma_1}" for gamma_1 in gamma_1_list],
             columns=pd.MultiIndex.from_product(
                     ([f"gamma_2 = {gamma_2}" for gamma_2 in gamma_2_list],
                      [f"gamma_3 = {gamma_3}" for gamma_3 in gamma_3_list]
            ...
            .to_csv(err_dir + f"{pde}_3layer_rel_l2_table.csv")